  ...
  return (
    <div className="flex flex-col min-h-screen">
      <main className="flex-1 px-6 py-12 max-w-2xl mx-auto w-full space-y-10">
        <h1 className="text-2xl" style={{ fontFamily: "Georgia, serif" }}>Admin</h1>

        <div className="grid grid-cols-2 gap-4">
          {stats.map((s) => (
            <Link key={s.href} href={s.href}
              className="border border-[#2C2A28]/10 p-5 hover:border-[#7A9E7E]/40 transition-colors"
            >
              <p className="text-3xl font-light mb-1">{s.count}</p>
              <p className="text-xs text-[#2C2A28]/50 uppercase tracking-wide">{s.label}</p>
            </Link>
          ))}
        </div>

        <nav className="space-y-2 text-sm">
          <Link href="/admin/applications" className="block text-[#7A9E7E] underline">Applications queue →</Link>
          <Link href="/admin/cohorts" className="block text-[#7A9E7E] underline">Cohorts →</Link>
          <Link href="/admin/members" className="block text-[#7A9E7E] underline">Members →</Link>
          <Link href="/admin/reports" className="block text-[#7A9E7E] underline">Reports →</Link>
        </nav>
      </main>
      <CrisisBanner />
    </div>
